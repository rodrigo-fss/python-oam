...
class IsolationPath(ScoringBaseClass):
    ...
    def score(self, dataframe: pd.DataFrame, query_point_index: int) -> float:
        ...
        for _ in range(self.number_of_paths):
            subsample = self._subsample_dataframe_with_query(dataframe, query_point_index)

            done_paths_length.append(self._calc_path_length(subsample, len(subsample) - 1))
    ...
    def _calc_path_length(self, subspace: pd.DataFrame, query_point_index: int) -> int:
        ...
        while len(subspace_sample) > 1:
            random_attribute = self._get_random_attribute(subspace_sample)

            # if the attribute can't be cut anymore, remove it
            if random_attribute.max == random_attribute.min:
                subspace_sample = subspace_sample.drop(random_attribute.name, axis=1)
                ...
                if len(subspace_sample.columns) == 0:
                    ...
                    path_length = self._path_length_adjustment(path_length, sample_size)
                    break
                continue

            subspace_sample = self._cut_tree(subspace_sample, random_attribute, query_point_index)
    ...
    def _get_random_attribute(self, subspace_sample: pd.DataFrame) -> types.SimpleNamespace:
        ...
        its metadata"""
        random_index = numpy.random.randint(low=0, high=len(subspace_sample.columns), size=None)
    ...
    def _cut_tree(
        self, subspace_sample: pd.DataFrame, random_attribute: types.SimpleNamespace, query_point_index: int
    ) -> pd.DataFrame:
        ...
        split_point = numpy.random.uniform(random_attribute.min, random_attribute.max, None)
